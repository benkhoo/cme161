...
d3.json("https://tranquil-peak-82564.herokuapp.com/api/v1.0/data/iris/", function(remote_json){
  ...
  var sepal_width_chart = dc
    ...
    .centerBar(true)
    .x( d3.scale.linear().domain([1,10]) ) //fix domain
    .xUnits(dc.units.fp.precision(.5));
  ...
  var petal_length_chart = dc
    ...
    .centerBar(true)
    .x( d3.scale.linear().domain([0,10]) ) //fix domain
    .xUnits(dc.units.fp.precision(.5));
  ...
  var petal_width_chart = dc
    ...
    .centerBar(true)
    .x( d3.scale.linear().domain([0.5,4]) ) //fix domain
    .xUnits(dc.units.fp.precision(.5));
  ...
  */
  ...
  if( sepal_length_chart.filters().length > 0 || 
      ...
      species_chart.filters().length > 0 ){
   d3.select(".btn-btn")
      .remove();
